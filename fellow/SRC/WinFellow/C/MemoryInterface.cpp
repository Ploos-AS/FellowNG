...
constexpr unsigned int EMEM_MAXARDS = 4;
...
constexpr uint32_t MEMORY_DMEM_OFFSET = 0xf40000;
...
void memoryEmemCardAdd(memoryEmemCardInitFunc cardinit, memoryEmemCardMapFunc cardmap)
{
...
void memoryEmemMap()
{
  ...
  {
    memoryBankSet(memoryEmemReadByte, memoryEmemReadWord, memoryEmemReadLong, memoryEmemWriteByte, memoryEmemWriteWord, memoryEmemWriteLong, nullptr, 0xe8, 0xe8, FALSE);
  }
}
...
void memoryDmemMap()
{
  ...
  {
    memoryBankSet(memoryDmemReadByte, memoryDmemReadWord, memoryDmemReadLong, memoryDmemWriteByte, memoryDmemWriteWord, memoryDmemWriteLong, memory_dmem, bank, bank, FALSE);
  }
}
...
void memoryChipMap(bool overlay)
{
  ...
  {
    memoryBankSet(memoryChipReadByte, memoryChipReadWord, memoryChipReadLong, memoryChipWriteByte, memoryChipWriteWord, memoryChipWriteLong, memory_chip, bank, 0, TRUE);
  }
  ...
  if (!chipsetGetECS())
  {
    ...
    for (uint32_t i = 1; i < 4; ++i)
    {
      ...
      {
        memoryBankSet(
            memoryChipReadByte, memoryChipReadWord, memoryChipReadLong, memoryChipWriteByte, memoryChipWriteWord, memoryChipWriteLong, memory_chip, bank, bank_start, TRUE);
      }
    }
  }
}
...
{
  memoryBankSet(
      memoryMysteryReadByte, memoryMysteryReadWord, memoryMysteryReadLong, memoryMysteryWriteByte, memoryMysteryWriteWord, memoryMysteryWriteLong, nullptr, 0xe9, 0, FALSE);

  memoryBankSet(
      memoryMysteryReadByte, memoryMysteryReadWord, memoryMysteryReadLong, memoryMysteryWriteByte, memoryMysteryWriteWord, memoryMysteryWriteLong, nullptr, 0xde, 0, FALSE);
}
...
void memoryIoMap()
{
  ...
  {
    memoryBankSet(memoryIoReadByte, memoryIoReadWord, memoryIoReadLong, memoryIoWriteByte, memoryIoWriteWord, memoryIoWriteLong, nullptr, bank, 0, FALSE);
  }
}
...
void memoryKickError(MemoryRomError errorcode, uint32_t data)
{
  ...
  switch (errorcode)
  {
    case MemoryRomError::MEMORY_ROM_ERROR_SIZE: sprintf(error3, "Illegal size: %u bytes, size must be either 8kB (A1000 bootstrap ROM), 256kB or 512kB.", data); break;
    case MemoryRomError::MEMORY_ROM_ERROR_AMIROM_VERSION: sprintf(error3, "Unsupported encryption method, version found was %u", data); break;
    case MemoryRomError::MEMORY_ROM_ERROR_AMIROM_READ: sprintf(error3, "Read error in encrypted Kickstart or keyfile"); break;
    case MemoryRomError::MEMORY_ROM_ERROR_KEYFILE: sprintf(error3, "Unable to access keyfile %s", memory_key); break;
    case MemoryRomError::MEMORY_ROM_ERROR_EXISTS_NOT: sprintf(error3, "File does not exist"); break;
    case MemoryRomError::MEMORY_ROM_ERROR_FILE: sprintf(error3, "File is a directory"); break;
    case MemoryRomError::MEMORY_ROM_ERROR_KICKDISK_NOT: sprintf(error3, "The ADF-image is not a kickdisk"); break;
    case MemoryRomError::MEMORY_ROM_ERROR_CHECKSUM: sprintf(error3, "The Kickstart image has a checksum error, checksum is %X", data); break;
    case MemoryRomError::MEMORY_ROM_ERROR_KICKDISK_SUPER: sprintf(error3, "The ADF-image contains a superkickstart. Fellow can not handle it."); break;
    case MemoryRomError::MEMORY_ROM_ERROR_BAD_BANK: sprintf(error3, "The ROM has a bad baseaddress: %X", memory_kickimage_basebank * 0x10000); break;
  }
...
void memoryKickOK()
{
  ...
  if (bVerifyChecksum && ((chksum = memoryKickChksum()) != 0))
    memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_CHECKSUM, chksum);
  else
  {
    ...
    if ((basebank == 0xf8) || (basebank == 0xfc))
    {
      ...
      memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_BAD_BANK, basebank);
  }
}
      ...
      if (pfnGetKey)
      {
        ...
        if (keysize)
        {
          ...
          if (keybuffer)
          {
            ...
            {
              if (!suppressgui) memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_KEYFILE, 0);
              return -1;
            }
          }
        }
...
int memoryKickLoadAF2(char *filename, FILE *F, uint8_t *memory_kick, const bool suppressgui)
{
  ...
  { /* Header seems OK */
    if (version != 1)
    {
      if (!suppressgui) memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_AMIROM_VERSION, version);
      return TRUE; /* File was handled */
    }
    else
    { /* Seems to be a file we can handle */
      fclose(F);
      uint32_t size = memoryKickDecodeAF(filename, memory_key, memory_kick, suppressgui);

      if (size == -1)
      {
        if (!suppressgui) memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_AMIROM_READ, 0);
        return TRUE;
      }

      if (size != 8192 && size != 262144 && size != 524288)
      {
        if (!suppressgui) memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_SIZE, size);
        return TRUE;
      }
      ...
      { /* Load A1000 bootstrap ROM */
        ...
        if (memory_a1000_bootstrap)
        {
          ...
          if (lCRC32 != 0x62F11C04)
          {
            free(memory_a1000_bootstrap);
            memory_a1000_bootstrap = nullptr;
            memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_CHECKSUM, lCRC32);
            return FALSE;
          }
        }
      }
...
void memoryKickDiskLoad(FILE *F)
{
  ...
  {
    memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_KICKDISK_NOT, 0);
    return;
  }
  ...
  {
    memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_KICKDISK_SUPER, 0);
    return;
  }
...
void memoryKickLoad()
{
  ...
  if (fileProperties == nullptr)
    memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_EXISTS_NOT, 0);
  else
  {
    if (fileProperties->Type != FileType::File)
      memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_FILE, 0);
    else
  ...
  if (!memory_kickimage_none)
  {
    ...
    { /* Normal kickstart image */
      ...
      { /* Rom size is wrong */
        memoryKickError(MemoryRomError::MEMORY_ROM_ERROR_SIZE, memory_kickimage_size);
      }
