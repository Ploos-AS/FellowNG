...
static long dos_errno()
{
  ...
  switch (e)
  {
    ...
    default: TRACE(("Unimplemented error %s\n", strerror(e))); return ERROR_NOT_IMPLEMENTED;
  }
}
/* FELLOW IN (START)---------------- */
#endif
...
{
  if (uip->hf.fd != 0) fclose(uip->hf.fd);
  if (uip->volname != 0) free(uip->volname);
  if (uip->devname != 0) free(uip->devname);
  if (uip->rootdir != 0) free(uip->rootdir);
  if (uip->unit_pipe) free(uip->unit_pipe);
  if (uip->back_pipe) free(uip->back_pipe);
    ...
    int *blocksize)
{
  ...
  if (nr >= mountinfo->num_units) return "No slot allocated for this unit";
...
static const char *set_filesys_unit_1(
    struct uaedev_mount_info *mountinfo, int nr, char *volname, char *rootdir, int readonly, int secspertrack, int surfaces, int reserved, int blocksize)
{
  ...
  else
  {
    ...
    if (ui->hf.fd == 0) return "Hardfile not found";

    if (secspertrack < 1 || secspertrack > 32767 || surfaces < 1 || surfaces > 1023 || reserved < 0 || reserved > 1023 || (blocksize & (blocksize - 1)) != 0)
    {
...
const char *set_filesys_unit(
    struct uaedev_mount_info *mountinfo, int nr, char *volname, char *rootdir, int readonly, int secspertrack, int surfaces, int reserved, int blocksize)
{
  UnitInfo ui = mountinfo->ui[nr];
  const char *result = set_filesys_unit_1(mountinfo, nr, volname, rootdir, readonly, secspertrack, surfaces, reserved, blocksize);
  if (result)
    mountinfo->ui[nr] = ui;
  else
    close_filesys_unit(&ui);
...
const char *add_filesys_unit(struct uaedev_mount_info *mountinfo, char *volname, char *rootdir, int readonly, int secspertrack, int surfaces, int reserved, int blocksize)
{
  const char *retval;
  int nr = mountinfo->num_units;
  UnitInfo *uip = mountinfo->ui + nr;

  if (nr >= MAX_UNITS) return "Maximum number of file systems mounted";

  mountinfo->num_units++;
  retval = set_filesys_unit_1(mountinfo, nr, volname, rootdir, readonly, secspertrack, surfaces, reserved, blocksize);
  if (retval) mountinfo->num_units--;
  return retval;
}
...
int kill_filesys_unit(struct uaedev_mount_info *mountinfo, int nr)
{
  ...
  if (nr >= mountinfo->num_units || nr < 0) return -1;
...
int move_filesys_unit(struct uaedev_mount_info *mountinfo, int nr, int to)
{
  ...
  if (nr >= mountinfo->num_units || nr < 0 || to >= mountinfo->num_units || to < 0 || to == nr) return -1;
  tmpui = uip[nr];
  if (to > nr)
  {
    int i;
...
{
  UnitInfo *uip = mountinfo->ui;
  if (num >= mountinfo->num_units) return -1;

  if (uip[num].volname != 0)
    sprintf(buffer, "(%s%d:) Filesystem, %s: %s %s", ffilesysGetDeviceNamePrefix().c_str(), num, uip[num].volname, uip[num].rootdir, uip[num].readonly ? "ro" : "");
  else
    sprintf(buffer, "(%s%d:) Hardfile, \"%s\", size %u bytes", ffilesysGetDeviceNamePrefix().c_str(), num, uip[num].rootdir, uip[num].hf.size);
  return 0;
}

void write_filesys_config(struct uaedev_mount_info *mountinfo, const char *unexpanded, const char *default_path, FILE *f)
{
  UnitInfo *uip = mountinfo->ui;
  int i;
  ...
  for (i = 0; i < mountinfo->num_units; i++)
  {
    ...
    if (uip[i].volname != 0)
    {
      fprintf(f, "filesystem=%s,%s:%s\n", uip[i].readonly ? "ro" : "rw", uip[i].volname, str);
    }
    else
    {
      fprintf(
          f, "hardfile=%s,%d,%d,%d,%d,%s\n", uip[i].readonly ? "ro" : "rw", uip[i].hf.secspertrack, uip[i].hf.surfaces, uip[i].hf.reservedblocks, uip[i].hf.blocksize, str);
    }
    free(str);
  }
}
...
struct uaedev_mount_info *dup_mountinfo(struct uaedev_mount_info *mip)
{
  ...
  for (i = 0; i < i2->num_units; i++)
  {
    ...
    if (uip->volname) uip->volname = my_strdup(uip->volname);
    if (uip->rootdir) uip->rootdir = my_strdup(uip->rootdir);
    if (uip->hf.fd) uip->hf.fd = fdopen(dup(fileno(uip->hf.fd)), uip->readonly ? "rb" : "r+b");
  }
  return i2;
}
...
struct hardfiledata *get_hardfile_data(int nr)
{
  ...
  if (nr < 0 || nr >= current_mountinfo->num_units || uip[nr].volname != 0) return 0;
  return &uip[nr].hf;
}
...
#define PUT_PCK_RES1(p, v)                                                                                                                                                    \
  do                                                                                                                                                                          \
  {                                                                                                                                                                           \
    do_put_mem_long((uae_u32 *)((p) + dp_Res1), (v));                                                                                                                         \
  } while (0)
#define PUT_PCK_RES2(p, v)                                                                                                                                                    \
  do                                                                                                                                                                          \
  {                                                                                                                                                                           \
    do_put_mem_long((uae_u32 *)((p) + dp_Res2), (v));                                                                                                                         \
  } while (0)
...
static char *bstr_cut(Unit *unit, uaecptr addr)
{
  ...
  for (i = 0; i < n; i++, addr++)
  {
    ...
    unit->tmpbuf3[i] = c;
    if (c == '/' || (c == ':' && colon_seen++ == 0)) p = unit->tmpbuf3 + i + 1;
  }
  unit->tmpbuf3[i] = 0;
  return p;
}
...
{
  if (aino->next == 0 || aino == &unit->rootnode) return;
...
static void dispose_aino(Unit *unit, a_inode **aip, a_inode *aino)
{
  ...
  if (unit->aino_hash[hash] == aino) unit->aino_hash[hash] = 0;

  if (aino->dirty && aino->parent) fsdb_dir_writeback(aino->parent);

  *aip = aino->sibling;
  if (aino->comment) free(aino->comment);
  free(aino->nname);
  free(aino->aname);
  free(aino);
}
...
{
  if (new_aino->dir || new_aino->shlock > 0 || new_aino->elock || new_aino == &unit->rootnode) /* Still in use */
    return;
    ...
    while (i < 50)
    {
      ...
      if (!parent->locked_children)
        for (;;)
        {
          ...
          {
            if (aino->shlock > 0 || aino->elock) write_log("panic: freeing locked a_inode!\n");
...
static void update_child_names(Unit *unit, a_inode *a, a_inode *parent)
{
  ...
  while (a != 0)
  {
    ...
    free(a->nname);
    a->nname = new_name;
    if (a->child) update_child_names(unit, a->child, a);
    a = a->sibling;
  }
}
...
static a_inode *lookup_aino(Unit *unit, uae_u32 uniq)
{
  ...
  if (uniq == 0) return &unit->rootnode;
...
static char *get_nname(Unit *unit, a_inode *base, char *rel, char **modified_rel)
{
  char *found;
  char *p = 0;
  ...
  *modified_rel = 0;
  ...
  if (fsdb_used_as_nname(base, rel)) return 0;
  ...
  if (found == rel) return build_nname(base->nname, rel);
...
static a_inode *new_child_aino(Unit *unit, a_inode *base, char *rel)
{
  ...
  if (aino == 0)
  {
    ...
    if (aino->dir) fsdb_clean_dir(aino);
  }
...
static a_inode *lookup_child_aino(Unit *unit, a_inode *base, char *rel, uae_u32 *err)
{
  ...
  while (c != 0)
  {
    ...
    if (l0 <= l1 && same_aname(rel, c->aname + l1 - l0) && (l0 == l1 || c->aname[l1 - l0 - 1] == '/')) break;
    c = c->sibling;
  }
  if (c != 0) return c;
  c = new_child_aino(unit, base, rel);
  if (c == 0) *err = ERROR_OBJECT_NOT_AROUND;
  return c;
}
...
static a_inode *lookup_child_aino_for_exnext(Unit *unit, a_inode *base, char *rel, uae_u32 *err)
{
  ...
  *err = 0;
  while (c != 0)
  {
    ...
    if (l0 <= l1 && strcmp(rel, c->nname + l1 - l0) == 0 && (l0 == l1 || c->nname[l1 - l0 - 1] == FSDB_DIR_SEPARATOR)) break;
    c = c->sibling;
  }
  if (c != 0) return c;
...
static a_inode *get_aino(Unit *unit, a_inode *base, const char *rel, uae_u32 *err)
{
  ...
  *err = 0;
  ...
  while (*p)
  {
    ...
    if (*p == '/')
    {
      if (curr->parent != 0) curr = curr->parent;
      p++;
    }
    else
    {
...
static uae_u32 startup_handler()
{
  ...
  for (i = 0; i < current_mountinfo->num_units; i++)
  {
    ...
    if (current_mountinfo->ui[i].volname == 0) continue;

    if (current_mountinfo->ui[i].startup == arg2) break;
  }

  if (i == current_mountinfo->num_units || access(current_mountinfo->ui[i].rootdir, R_OK) != 0)
  {
...
static void do_info(Unit *unit, dpacket packet, uaecptr info)
{
  struct fs_usage fsu;
...
static void action_disk_info(Unit *unit, dpacket packet)
{
  TRACE(("ACTION_DISK_INFO\n"));
  do_info(unit, packet, GET_PCK_ARG1(packet) << 2);
}

static void action_info(Unit *unit, dpacket packet)
{
  TRACE(("ACTION_INFO\n"));
...
static void dumplock(Unit *unit, uaecptr lock)
{
  ...
  TRACE(
      ("{ next=0x%lx, mode=%ld, handler=0x%lx, volume=0x%lx, aino %lx ",
       get_long(lock) << 2,
       get_long(lock + 8),
       get_long(lock + 12),
       get_long(lock + 16),
...
static void action_lock(Unit *unit, dpacket packet)
{
...
static ExamineKey *new_exkey(Unit *unit, a_inode *aino)
{
  ...
  {
    /* Did we find a free one? */
    if (ek->aino == 0) continue;
    if (ek->uniq < oldest) oldest = (oldest_ek = ek)->uniq;
  }
  ek = unit->examine_keys;
  for (i = 0; i < EXKEYS; i++, ek++)
  {
    /* Did we find a free one? */
    if (ek->aino == 0) goto found;
  }
...
static void move_exkeys(Unit *unit, a_inode *from, a_inode *to)
{
  ...
  if (tmp != from->exnext_count) write_log("filesys.c: Bug in ExNext bookkeeping.  BAD.\n");
...
static void action_examine_object(Unit *unit, dpacket packet)
{
  ...
  if (lock != 0) aino = lookup_aino(unit, get_long(lock + 4));
  if (aino == 0) aino = &unit->rootnode;
...
static void do_examine(Unit *unit, dpacket packet, ExamineKey *ek, uaecptr info)
{
  ...
  if (ek->curr_file == 0) goto no_more_entries;
...
static void action_examine_next(Unit *unit, dpacket packet)
{
  ...
  else if (uniq == 0xFFFFFFFF)
  {
    ...
    if (ek)
    {
      if (aino->exnext_count++ == 0) populate_directory(unit, aino);
    }
...
static void do_find(Unit *unit, dpacket packet, int mode, int create, int fallback)
{
  ...
  if (err == 0)
  {
    ...
    if (create != 2)
    {
      if ((((mode & aino->amigaos_mode) & A_FIBF_WRITE) != 0 || unit->ui.readonly) && fallback)
      {
        mode &= ~A_FIBF_WRITE;
      }
      ...
      if (((mode & aino->amigaos_mode) & A_FIBF_WRITE) != 0 || mode == 0)
      {
        PUT_PCK_RES1(packet, DOS_FALSE);
  ...
  openmode = (((mode & A_FIBF_READ) == 0 ? O_WRONLY : (mode & A_FIBF_WRITE) == 0 ? O_RDONLY : O_RDWR) | (create ? O_CREAT : 0) | (create == 2 ? O_TRUNC : 0));

  fd = open(aino->nname, openmode | O_BINARY, 0777);

  if (fd < 0)
  {
    if (aino_created) delete_aino(unit, aino);
    PUT_PCK_RES1(packet, DOS_FALSE);
    PUT_PCK_RES2(packet, dos_errno());
    return;
  }
...
static void action_fh_from_lock(Unit *unit, dpacket packet)
{
  uaecptr fh = GET_PCK_ARG1(packet) << 2;
  uaecptr lock = GET_PCK_ARG2(packet) << 2;
  a_inode *aino;
  ...
  prepare_for_open(aino->nname);
  TRACE(("  mode is %d\n", mode));
  openmode = (((mode & A_FIBF_READ) ? O_WRONLY : (mode & A_FIBF_WRITE) ? O_RDONLY : O_RDWR));

  /* the files on CD really can have the write-bit set.  */
  if (unit->ui.readonly) openmode = O_RDONLY;
...
static void action_find_input(Unit *unit, dpacket packet)
{
  do_find(unit, packet, A_FIBF_READ | A_FIBF_WRITE, 0, 1);
}

static void action_find_output(Unit *unit, dpacket packet)
{
  if (unit->ui.readonly)
  {
    PUT_PCK_RES1(packet, DOS_FALSE);
...
static void action_find_write(Unit *unit, dpacket packet)
{
  if (unit->ui.readonly)
  {
    PUT_PCK_RES1(packet, DOS_FALSE);
...
static void action_end(Unit *unit, dpacket packet)
{
  Key *k;
...
static void action_read(Unit *unit, dpacket packet)
{
  ...
  if (k->file_pos == 0 && size >= 4)
  {
    ...
    if (buf[0] == 0 && buf[1] == 0 && buf[2] == 3 && buf[3] == 0xF3) possible_loadseg();
  }
#endif
...
static void action_write(Unit *unit, dpacket packet)
{
  Key *k = lookup_key(unit, GET_PCK_ARG1(packet));
  uaecptr addr = GET_PCK_ARG2(packet);
  ...
  if (GET_PCK_RES1(packet) != size) PUT_PCK_RES2(packet, dos_errno());
  if (GET_PCK_RES1(packet) >= 0) k->file_pos += GET_PCK_RES1(packet);

  free(buf);
}

static void action_seek(Unit *unit, dpacket packet)
{
  Key *k = lookup_key(unit, GET_PCK_ARG1(packet));
...
static void action_set_protect(Unit *unit, dpacket packet)
{
  uaecptr lock = GET_PCK_ARG2(packet) << 2;
  uaecptr name = GET_PCK_ARG3(packet) << 2;
...
static void action_set_comment(Unit *unit, dpacket packet)
{
  ...
  maybe_free_and_out:
    if (commented) free(commented);
    return;
  }
  PUT_PCK_RES1(packet, DOS_TRUE);
  PUT_PCK_RES2(packet, 0);
  if (a->comment == 0 && commented == 0) goto maybe_free_and_out;
  if (a->comment != 0 && commented != 0 && strcmp(a->comment, commented) == 0) goto maybe_free_and_out;
  if (a->comment) free(a->comment);
  a->comment = commented;
  a->dirty = 1;
}

static void action_same_lock(Unit *unit, dpacket packet)
{
...
static void action_change_mode(Unit *unit, dpacket packet)
{
...
#define CHANGE_FH 1
  ...
  /* @@@ Brian: shouldn't this be good enough to support
     CHANGE_FH?  */
  if (type == CHANGE_FH) mode = (mode == 1006 ? -1 : -2);
...
static void action_parent_common(Unit *unit, dpacket packet, unsigned long uniq)
{
...
static void action_parent_fh(Unit *unit, dpacket packet)
{
  Key *k = lookup_key(unit, GET_PCK_ARG1(packet));
  if (!k)
  {
...
static void action_parent(Unit *unit, dpacket packet)
{
  uaecptr lock = GET_PCK_ARG1(packet) << 2;
...
static void action_create_dir(Unit *unit, dpacket packet)
{
  uaecptr lock = GET_PCK_ARG1(packet) << 2;
  uaecptr name = GET_PCK_ARG2(packet) << 2;
  a_inode *aino;
...
static void action_examine_fh(Unit *unit, dpacket packet)
{
  Key *k;
  a_inode *aino = 0;
  uaecptr info = GET_PCK_ARG2(packet) << 2;
...
static void action_set_file_size(Unit *unit, dpacket packet)
{
  Key *k, *k1;
  ...
  for (k1 = unit->keys; k1; k1 = k1->next)
  {
    ...
    {
      if (k1->file_pos > offset) offset = k1->file_pos;
    }
  }
  ...
  if (k->file_pos > offset) k->file_pos = offset;
...
static void action_delete_object(Unit *unit, dpacket packet)
{
  uaecptr lock = GET_PCK_ARG1(packet) << 2;
  uaecptr name = GET_PCK_ARG2(packet) << 2;
  a_inode *a;
...
static void action_set_date(Unit *unit, dpacket packet)
{
  ...
  a = find_aino(unit, lock, bstr(unit, name), &err);
  if (err == 0 && utime(a->nname, &ut) == -1) err = dos_errno();
  if (err != 0)
  {
    PUT_PCK_RES1(packet, DOS_FALSE);
    PUT_PCK_RES2(packet, err);
...
static void action_current_volume(Unit *unit, dpacket packet)
{
  PUT_PCK_RES1(packet, unit->volume >> 2);
}

static void action_rename_disk(Unit *unit, dpacket packet)
{
...
static void action_is_filesystem(Unit *unit, dpacket packet)
{
  PUT_PCK_RES1(packet, DOS_TRUE);
}

static void action_flush(Unit *unit, dpacket packet)
{
...
static uae_u32 exter_int_helper()
{
  ...
  switch (m68k_dreg(regs, 0))
  {
    case 0:
      ...
      {
        if (uae_sem_trywait(&singlethread_int_sem) != 0) /* Pretend it isn't for us. We might get it again later. */
          return 0;
    ...
    case 2:
      ...
      while (comm_pipe_has_data(&native2amiga_pending))
      {
        switch (read_comm_pipe_int_blocking(&native2amiga_pending))
        {
          ...
          case 2: /* ReplyMsg() */ m68k_areg(regs, 1) = read_comm_pipe_u32_blocking(&native2amiga_pending); return 3;

          default: write_log("exter_int_helper: unknown native action\n"); break;
        }
      }
#endif
      ...
      for (;;)
      {
        if (unit_no >= current_mountinfo->num_units) return 0;

        if (uip[unit_no].self != 0 && uip[unit_no].self->cmds_acked == uip[unit_no].self->cmds_complete && uip[unit_no].self->cmds_acked != uip[unit_no].self->cmds_sent)
          break;
        unit_no++;
      }
    ...
    default: write_log("Shouldn't happen in exter_int_helper.\n"); break;
  }
  return 0;
}
...
static int handle_packet(Unit *unit, dpacket pck)
{
  ...
  switch (type)
  {
    ...
    default: TRACE(("*** UNSUPPORTED PACKET %ld\n", type)); return 0;
  }
  return 1;
}
...
static void *filesys_thread(void *unit_v)
{
  ...
  for (;;)
  {
    ...
    uae_int_requested = 1;
    /* Send back the locks. */
    if (get_long(ui->self->locklist) != 0) write_comm_pipe_int(ui->back_pipe, (int)(get_long(ui->self->locklist)), 0);
...
static uae_u32 filesys_diagentry()
{
  ...
  {
    if (get_word(tmp) == 0x4AFC && get_long(tmp + 0x2) == tmp)
    {
