...
#define CONVERT_BLOCKS(B) adjust_blocks((B), fsd.f_fsize, 512)
  ...
  if (statfs(path, &fsd, sizeof(struct statfs)) != 0) return -1;
...
#define f_bavail f_bfree
  ...
  if (statfs(path, &fsd, sizeof fsd, 0) < 0) return -1;
...
struct statfs *fsb;
{
  ...
  if (dustat(stats.st_dev, 0, &fsd, sizeof(fsd))) return -1;
  fsb->f_type = 0;
